...
base::Time TimeLimitedWords::words_v1_sunset_day_;
...
base::Time TimeLimitedWords::GetWordsV1SunsetDay() {
  if (words_v1_sunset_day_.is_null()) {
    bool convert_result =
        base::Time::FromUTCString(kWordsv1SunsetDate, &words_v1_sunset_day_);
...
int TimeLimitedWords::GetRoundedDaysDiff(const base::Time& time1,
...
std::string TimeLimitedWords::GenerateForNow(const std::string& pure_words) {
  return TimeLimitedWords::GenerateForDate(pure_words, base::Time::Now());
}
